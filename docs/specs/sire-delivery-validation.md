...
| **Incidents history + unified Incidents nav** | ✅ | PR #13 — fixes the gap where past (RESOLVED/CLOSED) incidents were invisible and mobile had no Incidents nav (only "Declare Incident"). `GET /v1/incidents` made **param-additive** (`status`/`from`/`to`/`q`/`limit`) — **no-params = byte-identical legacy behaviour** (mobile banner / ZoneStatusBoard / dashboard /zones unaffected; backward-compatible by construction). Dashboard `/incidents` rebuilt: Active / Past (text search + time-range presets + status chips + count + Clear-all, industry-standard filter pattern) / Scheduled (upcoming drills — the only future-scheduled incident-type entity; reuses `/v1/drill-sessions`). Mobile: new `IncidentsListScreen` (same tabs) + distinct **"Incidents"** drawer entry (keeps "Declare Incident" separate). No migration/worker; additive |
