...
CREATE POLICY venues_update ON venues
  FOR UPDATE USING (id = current_venue_id() AND current_role() IN ('SH', 'DSH') OR is_sc_ops());
...
CREATE POLICY floors_insert ON floors
  FOR INSERT WITH CHECK (venue_id = current_venue_id() AND current_role() IN ('SH', 'DSH') OR is_sc_ops());

CREATE POLICY floors_update ON floors
  FOR UPDATE USING (venue_id = current_venue_id() AND current_role() IN ('SH', 'DSH') OR is_sc_ops());
...
CREATE POLICY zones_update ON zones
  FOR UPDATE USING (venue_id = current_venue_id() AND current_role() IN ('SH', 'DSH', 'SHIFT_COMMANDER', 'GM') OR is_sc_ops());
...
CREATE POLICY staff_insert ON staff
  FOR INSERT WITH CHECK (venue_id = current_venue_id() AND current_role() = 'SH' OR is_sc_ops());

CREATE POLICY staff_update ON staff
  FOR UPDATE USING (
    (venue_id = current_venue_id() AND (id = current_staff_id() OR current_role() = 'SH'))
    OR is_sc_ops()
  );
...
CREATE POLICY staff_certs_insert ON staff_certifications
  FOR INSERT WITH CHECK (venue_id = current_venue_id() AND current_role() IN ('SH', 'DSH'));
...
CREATE POLICY shift_instances_insert ON shift_instances
  FOR INSERT WITH CHECK (venue_id = current_venue_id() AND current_role() IN ('SH', 'DSH', 'SHIFT_COMMANDER'));

CREATE POLICY shift_instances_update ON shift_instances
  FOR UPDATE USING (venue_id = current_venue_id() AND current_role() IN ('SH', 'DSH', 'SHIFT_COMMANDER'));
...
CREATE POLICY zone_assignments_delete ON staff_zone_assignments
  FOR DELETE USING (venue_id = current_venue_id() AND current_role() IN ('SH', 'DSH', 'SHIFT_COMMANDER'));
...
CREATE POLICY tasks_select ON task_instances
  FOR SELECT USING (
    ...
    AND (
      ...
      OR current_role() IN ('SH', 'DSH', 'SHIFT_COMMANDER', 'GM', 'AUDITOR')
    )
  );
...
CREATE POLICY incidents_insert ON incidents
  FOR INSERT WITH CHECK (
    venue_id = current_venue_id()
    AND current_role() IN ('SH', 'DSH', 'SHIFT_COMMANDER', 'GM', 'FM')
  );

CREATE POLICY incidents_update ON incidents
  FOR UPDATE USING (
    venue_id = current_venue_id()
    AND current_role() IN ('SH', 'DSH', 'SHIFT_COMMANDER', 'GM')
  );
...
CREATE POLICY equipment_insert ON equipment_items
  FOR INSERT WITH CHECK (venue_id = current_venue_id() AND current_role() IN ('SH', 'DSH', 'FM'));

CREATE POLICY equipment_update ON equipment_items
  FOR UPDATE USING (venue_id = current_venue_id() AND current_role() IN ('SH', 'DSH', 'FM'));
...
CREATE POLICY audit_select ON audit_logs
  FOR SELECT USING (venue_id = current_venue_id() AND current_role() IN ('SH', 'AUDITOR') OR is_sc_ops());
...
CREATE POLICY vms_ep_update ON vms_entry_points
  FOR UPDATE USING (venue_id = current_venue_id() AND current_role() IN ('SH', 'DSH'));
...
CREATE POLICY vms_visits_update ON vms_visit_records
  FOR UPDATE USING (venue_id = current_venue_id() AND current_role() IN ('SH', 'DSH', 'GROUND_STAFF', 'FLOOR_SUPERVISOR'));
