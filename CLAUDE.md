...
| **Shift Roster Wave (Phase 5.24; BR-AR pull-forward written 2026-05-20, founder-applied when ready)** | `nexus/specs/SafeCommand_ShiftRoster_Architecture_v1.md` (Forge, 2026-05-20) + `nexus/specs/2026-05-20_shift-roster-requirements.md` (Prime v1.1) + ADR-0007 + ADR 0001 (2026-05-20 amendment) | 11 new BRs (BR-AK…BR-AU): recurring roster patterns + working-days/weekly-off + rotation cycle library + leave/unavailability + auto-materialisation worker + shift-swap workflow + coverage rules + multi-shift flexibility (BR-AR) + MBV-aware + RACI + compliance export PDF. **BR-AR pulled forward as wave 1** (`021_shifts_multi_shift_breaks.sql` written; ALTER shifts: breaks JSONB + min_handover_minutes + description + is_overnight GENERATED + venue_type_default; ⏳ founder psql-apply; Aug 2026 per arch bandwidth — can be earlier). **BR-12 contract preserved verbatim** (ADR-0007): one internal refactor reads `shifts.min_handover_minutes` post mig 021. **The full pattern engine (BR-AK/AL/AM/AN/AO/AP/AQ/AS/AT/AU) = Phase 5.24 Q1 2028** (migs `022_roster_engine.sql` + `023_coverage_rules.sql`). Hard Rule 24 gates the BR-AR code (handover-service refactor + `/v1/shifts` field surface + Ops Console editor) on the mig 021 apply. |
