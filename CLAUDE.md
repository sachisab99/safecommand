...
| **Evacuation Map Studio + Standards-Closure (v9/v9.1; partial pull-forward shipped)** | BP v9 §9/§22 + Architecture **v9.1** §21+§22+§23 + `nexus/specs/SafeCommand_Architecture_v91_Complete.md` | 10 Map Studio BRs (BR-Q…BR-Z) + 10 standards-closure BRs (BR-AA…BR-AJ). Konva.js editor, IMDF-compatible GeoJSON canonical (EC-24, never proprietary), 7-jurisdiction validation, per-posting PDFs (Puppeteer), Live Mode on `incident_zone_states` (BR-W; SIRE is its dependency root). **Standards-Closure P1 subset (BR-AB Safety Committee + BR-AF AMC + BR-AG MSDS) PULLED FORWARD 2026-05-19**: mig **`020`** DEPLOYED + 3 API routes LIVE on main (`/v1/safety-committee` / `/v1/amc-contracts` / `/v1/msds`). Map Studio + LMS + drill-tabletop + NABH-QI view + the 2 deferred std-closure tables (BR-AA / BR-AD — FK floor_plans/evacuation_annotations) remain Phase 5.23 / Q4 2027+, **renumbered to 024+** per ADR 0001's next-free-integer rule (migs 021–023 now reclaimed by the shift-roster wave). Floor-plan currency amber @ 9mo / hard-block @ 12mo (EC-25). Hard Rule 24 + ADR 0001 (2026-05-19/20 amendments) govern. |
| **Shift Roster Wave (Phase 5.24; wave 1 BR-AR LIVE, wave 2 pattern-engine pull-forward authorised 2026-05-21)** | `nexus/specs/SafeCommand_ShiftRoster_Architecture_v1.md` (Forge) + `nexus/specs/2026-05-20_shift-roster-requirements.md` (Prime v1.1) + ADR-0007 + ADR 0001 (2026-05-20 / 2026-05-21 amendments) | 11 new BRs (BR-AK…BR-AU): recurring roster patterns + working-days/weekly-off + rotation cycle library + leave/unavailability + auto-materialisation worker + shift-swap workflow + coverage rules + multi-shift flexibility (BR-AR) + MBV-aware + RACI + compliance export PDF. **Wave 1 LIVE end-to-end** (mig `021` DEPLOYED 2026-05-20; api/action field surface + Ops Console BreaksEditor UI + dashboard read-surface all on main). **Wave 2 pattern-engine pull-forward authorised 2026-05-21**: migs `022_roster_engine.sql` (6 tables: rotation_cycle_library + 7 seeded rotations · roster_patterns · roster_cycle_positions · staff_roster_assignments · staff_unavailability with gist EXCLUDE · shift_swap_requests with in-row state + audit_logs precedent) + `023_coverage_rules.sql` (1 table; uses `staff_role_enum` from mig 001) — both WRITTEN, ⏳ founder psql-apply (apply 022 FIRST, then 023). **BR-12 contract preserved verbatim** (ADR-0007). Pre-deploy adaptations: building_id/building_visible omitted pre-MBV (mig 009 PENDING); swap-request FK references `staff_zone_assignments` (Reconciliation Flag #4); `btree_gist` extension auto-created. Hard Rule 24 gates the wave-2 code (pattern CRUD api + materialisation worker BR-AO + coverage validation + swap/leave workflows + UI) on the migs 022/023 apply. |
...
#### Shift Roster Wave (BR-AK to BR-AU) — Phase 5.24 NEW (2026-05-20); BR-AR pull-forward written

> Replaces the per-day bulk-assignment workflow with a published recurring-pattern roster engine. Spec: `nexus/specs/SafeCommand_ShiftRoster_Architecture_v1.md` (Forge, 2026-05-20) + `2026-05-20_shift-roster-requirements.md` (Prime v1.1) + **ADR-0007** (decouple handover from daily assignment — BR-12 contract preserved verbatim). **BR-AR pulled forward as wave 1** (mig 021 written 2026-05-20; founder psql-apply; Aug 2026 per arch bandwidth, can be earlier). Other 10 BRs = Phase 5.24 Q1 2028.

| ID | Title | Phase | Mig (per ADR 0001) | Standards source |
|---|---|---|---|---|
| BR-AK | Roster pattern entity (recurring publishable template) | 5.24 (Q1 2028) | 022 (`roster_patterns`) | NABH 6th HRM.4.a (published duty roster) |
| BR-AL | Per-staff working days + weekly-off (rotation-aware) | 5.24 | 022 (cols on `staff_roster_assignments`) | **Factories Act 1948 §52** (weekly holiday) + §51 (48-hr week) + §54 (9-hr day) |
| BR-AM | Rotation cycle library (4-on-2-off / 2-2-3 Pitman / weekly day-night / continental / 4-day-night-4-off / standard office / 6-day / custom) | 5.24 | 022 (`rotation_cycle_library` — global, no RLS) | PSARA 2005 (12-hr rotating) + healthcare/industrial baselines |
| BR-AN | Leave / unavailability calendar (LEAVE_ANNUAL/SICK/TRAINING/PERSONAL + OFF_DUTY + SUSPENDED) | 5.24 | 022 (`staff_unavailability`) | Feeds the **repo** ADR-0004 drill reason taxonomy (LEAVE_*/OFF_DUTY) |
| BR-AO | Auto-materialisation worker — pattern → shift_instances + shift_assignments (idempotent, 30-day horizon default) | 5.24 | (no new schema — uses 022 patterns + existing instances) | Activates after ADR 0005 workers always-on (June 2026) |
| BR-AP | Shift-swap workflow — REQUESTED → COUNTERPART_ACCEPTED → APPROVED, append-only audit | 5.24 | 022 (`shift_swap_requests`) | Hard Rule 4 (append-only) |
| BR-AQ | Coverage rules + pre-publish gap validation + post-publish gap alerts (per building / zone / role / shift) | 5.24 | 023 (`coverage_rules`) | NABH 6th HRM.4.b (adequate per-shift staffing) + extends BR-34 (two-person zones) |
| **BR-AR** | **Multi-shift flexibility — multiple shifts per day, custom start/end times, optional breaks JSONB, min_handover_minutes, is_overnight GENERATED, venue_type_default** | **5.24 (wave 1 — pull-forward)** | **`021_shifts_multi_shift_breaks.sql`** (additive ALTER; **written 2026-05-20**; ⏳ founder psql-apply) | **Founder-specified** + Factories Act §55 (rest intervals → breaks) |
| BR-AS | MBV-aware roster scoping (`building_id` nullable everywhere per EC-16) | 5.24 | 022 (cols on patterns/assignments) | NFR-25 / EC-16 |
| BR-AT | Roster role permissions (RACI: GS/FS/SC/SH-DSH/GM/AUD/SC-Ops) + staff self-service for own roster + swap/leave | 5.24 | (RLS policies on 022/023) | BR-04 (8-role) |
| BR-AU | Published roster as compliance export PDF (NABH_HRM / FIRE_NOC_DUTY_ROSTER / INSURANCE_PACK / GENERIC) | 5.24 | (no new schema — reuses BR-Y Handlebars+Puppeteer pipeline) | NABH §HRM duty-roster + Fire NOC duty-roster + insurance pack |

**ADR-0007 (decouple handover from daily assignment):** BR-12 contract preserved verbatim at the API level (endpoints, payloads, status codes, `shift_handovers` schema all unchanged). The pattern engine (BR-AO) is just a new *producer* of `shift_instances` / `shift_assignments` rows. One internal refactor when mig 021 ships: the handover-pending notification reads `shifts.min_handover_minutes` (mig 021) instead of a hardcoded `15`. See `docs/adr/0007-decouple-handover-from-daily-assignment.md`.
...
- **ADR 0007 — Decouple shift handover from daily assignment (2026-05-20):** `docs/adr/0007-decouple-handover-from-daily-assignment.md` — gates BR-AR / Phase 5.24 roster wave; BR-12 contract preserved verbatim
- **Shift Roster Architecture v1.0 (Forge, 2026-05-20):** `../../nexus/specs/SafeCommand_ShiftRoster_Architecture_v1.md` (2674 lines, 11 BRs BR-AK…BR-AU, all 8 engineering refinements applied, 3 reconciliation flags resolved) — engineering authority for migs 021/022/023
- **Shift Roster Requirements v1.1 (Prime, 2026-05-20):** `../../nexus/specs/2026-05-20_shift-roster-requirements.md` (891 lines, business requirements + ADR-0007 source)
